...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SAIR_DRIVS_TableData]
...
BEGIN
    ...
    (
        (SELECT ', CAST(' +   QUOTENAME(name) + ' AS NVARCHAR(80)) AS ' + name 
            FROM sys.columns
            WHERE objecT_id IN (OBJECT_ID('drivervars.' + @tableName), OBJECT_ID('driverconst.' + @tableName))
    ...
    SET @tableDataSQL = 'SELECT  TAB.name TableName,
        ...
        (
            ...
            FROM ( SELECT ' + @columnsTable + ' FROM drivervars.' + @tableName + ' V JOIN driverconst.' + @tableName + ' C ON V.SAIR_VEHIID = C.CSAIR_VEHIID where SAIR_VEHIID = ' + CAST(@vehicleId AS NVARCHAR(60)) + ' ) AS P
            UNPIVOT

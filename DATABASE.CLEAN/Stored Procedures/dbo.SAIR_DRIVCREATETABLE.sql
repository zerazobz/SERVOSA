...
BEGIN
    ...
    SELECT @vehiclevarsSQL = 'create table [drivervars].' + QUOTENAME(@tableNormalizedName) + '(id int identity(1,1), SAIR_VEHIID int not null )';
    PRINT @vehiclevarsSQL
    SELECT @vehicleconstSQL = 'create table [driverconst].' + QUOTENAME(@tableNormalizedName) + '(cid int identity(1, 1), CSAIR_VEHIID int not null, DiasAlerta int null, RutaDocumento nvarchar(800), FechaVencimiento DateTime)';
    ...
    SELECT @foreignKeyVehicleVarsSQL = 'alter table [drivervars].' + QUOTENAME(@tableNormalizedName) + ' add constraint vehiclevar' + @tableNormalizedName
        + '_VEHI_FK foreign key (SAIR_VEHIID) REFERENCES dbo.SAIR_DRIVER(Codigo)';
    
    SELECT @foreignKeyVehicleConstSQL = 'alter table [driverconst].' + QUOTENAME(@tableNormalizedName) + ' add constraint vehicleconst' + @tableNormalizedName
        + '_VEHI_FK foreign key (CSAIR_VEHIID) REFERENCES dbo.SAIR_DRIVER(Codigo)';
    ...
    SELECT @primaryKeySQL = 'alter table [drivervars].' + QUOTENAME(@tableNormalizedName) + ' add constraint vehiclevars' + @tableNormalizedName
        + 'PK primary key(SAIR_VEHIID)';
    PRINT @primaryKeySQL;
    EXEC sp_executesql @primaryKeySQL;

    SELECT @primaryKeySQL = 'alter table [driverconst].' + QUOTENAME(@tableNormalizedName) + ' add constraint vehicleconst' + @tableNormalizedName
